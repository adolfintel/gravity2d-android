 ...
 * ~98% parallelization.<br>
 * 
 * THIS IS NOT THE SAME AS THE PC VERSION, IT USES SINGLE PRECISION CALCULATIONS!
 *
 * @author Federico
 */
public class Simulation extends Thread {
    ...
    public Simulation() {
        ...
        int nThreads = nCores; //unlike the PC version, this "shit edition" uses 1 thread per core instead of 2 to reduce lag
    ...
    private void handleCollisions() {
        while (!collisionCouples.isEmpty()) {
            ...
            float totalMass = 0, newX = 0, newY = 0, newVX = 0, newVY = 0, newDensity = 0;
            Point newP = null;
            ...
            for (Point p : currentCollisionGroup) {
                final float f = p.getMass() / totalMass;
                newX += p.getX() * f;
                newY += p.getY() * f;
                newVX += p.getVelX() * f;
                newVY += p.getVelY() * f;
